...
**Where it stands:** 49 of a planned 108 tools are built, tested and live.
...
`49 tools live` is not typed anywhere. Any element with `data-tool-count` is
filled in by `main.js` straight from the registry:

```html
<span data-tool-count="live">49</span> tools live
<span data-tool-count="remaining">59</span> on the way
```
...
Find-and-replace `?v=11` → `?v=12` across all 57 pages (currently `?v=11`,
277 occurrences), the template included.
...
**`test_tools.py` runs all 49, not just the one you changed.** That is the
...
**Live (49):** word-counter, case-converter, lorem-ipsum-generator,
...
text-to-morse, nato-phonetic-converter.
